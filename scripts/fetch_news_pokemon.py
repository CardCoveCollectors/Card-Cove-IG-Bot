"""
Pulls Pokemon TCG news headlines from the free Google News RSS search feed
configured in config_news_pokemon.py, filters for newsworthy keywords, and
returns fresh candidates newest-first.

No API key, no LLM call — pure RSS + keyword matching, so this costs
nothing to run.

Dedup against what's already been posted happens in main_news_pokemon.py
(it owns state_news_pokemon.json), not here — this module only knows
...
from config_news_pokemon import NEWS_FEEDS, MAX_AGE_HOURS, KEYWORDS, BLOCKLIST
...
def fetch_candidates():
    """Returns every fresh, relevant Pokemon TCG news item, newest first,
    deduped by normalized headline (Google News often surfaces the same
    story from several outlets)."""
    all_items = []
