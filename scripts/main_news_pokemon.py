"""
Orchestrator for the Pokemon TCG News pipeline. Run as two steps (see
.github/workflows/post_news_pokemon.yml):

  python main_news_pokemon.py build     -> checks the Pokemon TCG news feed
                                            for anything new + newsworthy
                                            that hasn't been posted before.
                                            If it finds something, renders
                                            the graphic + caption and
                                            writes posts/latest_news_pokemon.json.
  ...
  python main_news_pokemon.py publish   -> reads posts/latest_news_pokemon.json
                                            (written by `build`), builds the
                                            raw.githubusercontent.com URL
                                            for the image that was just
                                            pushed, and publishes it to
                                            Instagram.

Uses its own state file (state_news_pokemon.json, a set of normalized
headlines already posted) and is fully independent of the One Piece news
pipeline (main_news_onepiece.py / state_news_onepiece.json) so neither
...
from config_news_pokemon import BRAND_HANDLE, STATE_FILE
from posts_cleanup import cleanup_old_posts
from fetch_news_pokemon import fetch_candidates, dedup_key
from build_image_news import build_news_image
from caption_news import build_caption

ROOT = os.path.join(os.path.dirname(__file__), "..")
POSTS_DIR = os.path.join(ROOT, "posts")
LATEST_META = os.path.join(POSTS_DIR, "latest_news_pokemon.json")
...
def cmd_build(args):
    cleanup_old_posts(POSTS_DIR, "news-pokemon-")
    ...
    if not fresh:
        print(f"No new Pokemon TCG news found this run ({len(candidates)} candidate(s) seen, all already posted or none matched).")
        return
    ...
    filename = f"news-pokemon-{_slugify(item['title'])}-{timestamp}.png"
