...
def build_list_image(cards, list_type, set_name, out_path, brand_name="", n=10,
                      ...
                      logo_caption=None):
    """`logo_caption`, if given, is drawn centered directly under the set
    logo — for the Japanese pipeline, where the logo art itself is in
    Japanese and an English audience needs the real set name spelled out
    (e.g. "ABYSS EYE" under a Japanese wordmark). Left as None (the
    default) for English posts, where the logo is already in English and
    a caption under it would just be redundant."""
    cards = cards[:n]
    ...
    # ---- Header: large real set logo top-right (+ optional English name
    # caption centered under it), two-line title top-left ----
    ...
    if set_logo is not None:
        ...
        header_right_bottom = ly + set_logo.height

        if logo_caption:
            cap_f = font(FONT_MEDIUM, 24)
            cap_text, cap_f = fit_text(draw, logo_caption.upper(), FONT_MEDIUM, 24, set_logo.width + 40)
            cap_w = text_w(draw, cap_text, cap_f)
            cap_x = lx + (set_logo.width - cap_w) / 2
            cap_y = ly + set_logo.height + 10
            draw.text((cap_x + 1, cap_y + 1), cap_text, font=cap_f, fill=(0, 0, 0, 120))
            draw.text((cap_x, cap_y), cap_text, font=cap_f, fill=TEAL)
            header_right_bottom = cap_y + cap_f.size
    else:
        title_max_w = W - 2 * pad
    ...
    title_bottom = y2 + f2.size
    grid_top = max(title_bottom, header_right_bottom) + 36
