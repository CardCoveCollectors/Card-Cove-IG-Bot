...
# Podium (top 3) accent colors — gold / silver / bronze. Everything past
# #3 uses TEAL instead of gold so the podium still reads as special.
MEDAL_COLORS = {
    1: (255, 214, 79),
    2: (208, 213, 222),
    3: (216, 148, 92),
...
def build_list_image(cards, list_type, set_name, out_path, brand_name="", n=10,
    ...
    total_grid_h = row_content_h * rows + row_gap * (rows - 1)

    avail_h = H - grid_top - footer_reserve
    grid_top += max(0, (avail_h - total_grid_h) // 2)
    ...
    for i, card in enumerate(cards):
        ...
        # gold/silver/bronze; #4-10 use the logo's teal instead of gold so
        # the podium still stands out on its own.
        badge_color = MEDAL_COLORS.get(rank, TEAL)
        badge_text_fill = (20, 14, 8) if rank in MEDAL_COLORS else (8, 26, 30)
        ...
        name_text, nf = fit_text(draw, name, FONT_MEDIUM, 20, cell_w)
