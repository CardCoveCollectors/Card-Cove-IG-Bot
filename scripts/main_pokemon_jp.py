...
from fetch_prices_pokemon_jp import top_n_by_price, get_jp_sets, get_all_featured_set_codes, API_BASE
...
def _clean_logo_caption(set_name, set_code):
    """PokeWallet's set name sometimes comes back with the code baked in
    (e.g. "SV2a: Pokemon Card 151" or "M5: Abyss Eye") — strip that prefix
    so the caption under the logo just reads the plain English name."""
    cleaned = re.sub(rf"^{re.escape(set_code)}\s*:\s*", "", set_name, flags=re.I).strip()
    return cleaned or set_name
...
def cmd_build(args):
    cleanup_old_posts(POSTS_DIR, "pokejp-")
    ...
    for attempt in range(max_attempts):
        ...
        for list_type in ("most_expensive", "least_expensive"):
            ...
            build_list_image(
                ...
                cheapest_label="ARS",
                set_logo_url=f"{API_BASE}/sets/{set_code}/image?language=jap",
                logo_caption=_clean_logo_caption(set_name, set_code),
            )
