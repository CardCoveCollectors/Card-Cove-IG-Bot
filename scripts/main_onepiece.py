...
from posts_cleanup import cleanup_old_posts
from character_rotation import next_images
...
CHARACTER_ART_DIR = os.path.join(ROOT, "assets", "character_art")
CHARACTER_STATE_PATH = os.path.join(ROOT, "state_onepiece_charart.json")


_BOOSTER_PREFIX_RE = re.compile(r"^(extra|premium)\s+booster\s*[:\-]\s*", re.IGNORECASE)


def _clean_set_name(name):
    """optcgapi.com prefixes Extra Booster / Premium Booster set names
    with "Extra Booster: " / "Premium Booster: " (or " - "), which is
    redundant on the poster (the set code badge already makes clear
    it's a booster) and makes long titles wrap awkwardly. Strip it,
    keeping just the meaningful part, e.g. "One Piece Heroines Edition"
    instead of "Extra Booster: One Piece Heroines Edition"."""
    return _BOOSTER_PREFIX_RE.sub("", name).strip()


def _set_name(set_id):
    for s in get_all_sets():
        if s["set_id"] == set_id:
            return _clean_set_name(s["set_name"])
    return set_id
...
def _character_images_for_run():
    """Picks 2 DISTINCT character images for this run (one per list type)
    from the rotating pool via a no-repeat-until-exhausted shuffle, so
    the same character never covers both of the same day's posts. Falls
    back to no character art at all (returns {}) if the pool is missing
    or has fewer than 2 images yet — this feature is additive and should
    never block a post from going out."""
    try:
        picks = next_images(CHARACTER_ART_DIR, CHARACTER_STATE_PATH, count=2)
    except Exception as e:
        print(f"WARNING: character art rotation failed ({e}) — posting without it", file=sys.stderr)
        return {}
    if len(picks) < 2:
        return {}
    return {
        "most_expensive": os.path.join(CHARACTER_ART_DIR, picks[0]),
        "least_expensive": os.path.join(CHARACTER_ART_DIR, picks[1]),
    }


def cmd_build(args):
    cleanup_old_posts(POSTS_DIR, "op-")
    ...
    for attempt in range(len(FEATURED_SETS) + 1):
        ...
        char_images = _character_images_for_run()
        posts_meta = []
        ok = True
        for list_type in ("most_expensive", "least_expensive"):
            ...
            build_list_image(
                ...
                character_img_path=char_images.get(list_type),
            )
