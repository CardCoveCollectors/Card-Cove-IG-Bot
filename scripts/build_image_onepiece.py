...
def character_corner_flag(size, img_path):
    """A compact rounded-square 'trading card' style character portrait
    tucked in the top-left corner — echoes the card-grid aesthetic
    already used below. Only called when character_rotation.py actually
    found an image; no placeholder path here since the header simply
    reverts to the plain title-only layout when there's nothing to draw
    (see build_list_image)."""
    radius = int(size[0] * 0.12)
    canvas = Image.new("RGBA", size, (0, 0, 0, 0))
    mask = rounded_mask(size[0] - 8, size[1] - 8, radius=radius)
    art = Image.open(img_path).convert("RGBA")
    iw, ih = art.size
    target_w, target_h = size[0] - 8, size[1] - 8
    scale = max(target_w / iw, target_h / ih)
    art = art.resize((max(1, round(iw * scale)), max(1, round(ih * scale))), Image.LANCZOS)
    left = (art.width - target_w) // 2
    top = (art.height - target_h) // 2
    art = art.crop((left, top, left + target_w, top + target_h))
    canvas.paste(art, (4, 4), mask)
    return canvas


def set_code_badge(max_h, code_text, set_name=None):
    ...
    Shows the real set name (e.g. "The Time of Battle") when one is
    available, falling back to the generic "One Piece TCG" label only if
    the caller doesn't have one — set_name is already threaded through
    from main_onepiece.py's own set lookup, so in practice this will
    always be the real name. Kept in its natural title case rather than
    all-caps — reads better at this size."""
    pad_x = 34
    ...
    small_text = set_name or "One Piece TCG"
    small_bbox = dd.textbbox((0, 0), small_text, font=small_f)
    small_w, small_h = small_bbox[2] - small_bbox[0], small_bbox[3] - small_bbox[1]

    # Badge width now needs to fit the (possibly longer) set name too.
    w = max(main_w, small_w) + pad_x * 2
    h = max_h
...
def build_list_image(cards, list_type, set_name, out_path, brand_name="", n=10,
    ...
    # plus an optional rotating character-art flag to the left of the
    # title (English One Piece only — character_img_path is None for
    # every other pipeline, which reproduces the original text-only
    # layout exactly). ----
    badge = set_code_badge(150, set_id, set_name=set_name) if set_id else None
    if badge is not None:
        ...
        title_max_w_no_flag = lx - pad - 24
        title_right_bound = lx
    else:
        title_max_w_no_flag = W - 2 * pad
        title_right_bound = W - pad

    flag_size = (110, 150)
    if character_img_path:
        flag = character_corner_flag(flag_size, character_img_path)
        fx, fy = pad, top_y - 6
        paste_shadow(img, (fx, fy, fx + flag_size[0], fy + flag_size[1]), blur=8, opacity=90,
                     radius=int(flag_size[0] * 0.12), offset=(0, 4))
        img.paste(flag, (fx, fy), flag)
        title_x = pad + flag_size[0] + 20
        title_max_w = title_right_bound - title_x - 24
    else:
        title_x = pad
        title_max_w = title_max_w_no_flag
    ...
    title_block_h = f1.size + 8 + f2.size
    text_top = (top_y + (flag_size[1] - title_block_h) / 2) if character_img_path else top_y
    draw.text((title_x + 2, text_top + 2), t1, font=f1, fill=(0, 0, 0, 90))
    draw.text((title_x, text_top), t1, font=f1, fill=WHITE)
    y2 = text_top + f1.size + 8
    draw.text((title_x + 2, y2 + 2), t2, font=f2, fill=(0, 0, 0, 90))
    draw.text((title_x, y2), t2, font=f2, fill=WHITE)

    header_bottom = (top_y + flag_size[1]) if character_img_path else (y2 + f2.size)
    grid_top = header_bottom + 30
