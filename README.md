...
scripts/
  ...
  posts_cleanup.py            - SHARED: every pipeline's build step calls this with its own
                                 filename prefix (e.g. "poke-", "op-") to delete its own old
                                 post images before generating new ones, so posts/ (and the
                                 repo) doesn't grow forever. Never touches another
                                 pipeline's files.
